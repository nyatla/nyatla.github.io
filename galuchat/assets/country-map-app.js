...
const CONFIG = window.GALUCHAT_COUNTRY_MAP;
...
window.addEventListener("machml:localechange", () => {
  if (fatalError) showFatalMessage();
  else updateControls();
});
...
async function main() {
  if (!CONFIG) throw new Error("GALUCHAT_COUNTRY_MAP is not configured");
  const [displayReaders, wordbookBytes] = await Promise.all([
    loadMapsets(CONFIG.mapsets),
    fetchBinary(CONFIG.wordbook),
  ]);
  ...
  interactiveMap = new Galuchat.GaluchatInteractiveMap(shell, source, {
    center: source.snapCenter(CONFIG.center, CONFIG.initialLevelIndex),
    maxWidth: 640,
    maxHeight: 480,
  ...
  homeButton.addEventListener("click", () => setLocation(CONFIG.center));
  ...
  source.setZoomLevelIndex(CONFIG.initialLevelIndex);
  for (const button of [zoomOutButton, zoomInButton, homeButton, ...locationButtons]) button.disabled = false;
  updateControls();
...
function setLocation(center) {
  ...
  interactiveMap.setView({
    center: source.snapCenter(center, CONFIG.initialLevelIndex),
    selectedCode: null,
  });
}

function updateControls() {
  if (interactiveMap === null || source === null) return;
...
async function loadMapsets(mapsets) {
  const entries = await Promise.all(Object.entries(mapsets).map(async ([id, url]) => {
